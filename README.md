...
- When `spec.workloads[].csi.enabled=true`, the chart automatically creates `<workingDirectory>/.env` (default `/app/.env`) from mounted secret files (default mount path `/mnt/secrets`) using an init container.
