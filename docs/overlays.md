...
- When `spec.workloads[].csi.enabled=true`, the chart generates `<workingDirectory>/.env` (default `/app/.env`) from mounted secret files (from `csi.mountPath`, default `/mnt/secrets`) before the app container starts.
